...
#文件信息表 t_file (用户id , 大小 , 路径 , 引用计数 , MD5 , 上传完状态 , 文件类型 )
...
#用户文件关系表 t_user_file ( 用户id , 文件id , 目录 , 文件名字, 上传时间 , 分享码, 分享密码 , 分享时间 )
...
#1.联表查询 t_file 和t_user_file 的视图
#新建视图  user_file_info
...
#比如这个 张三 id 是1
#查询语句
select * from t_user where u_tel = '%s' and u_password = '%s';
select * from t_user where u_tel = '13312341234' and u_password = 'abc12345…';

#3.用户id为1的人, 上传一个文件 abc.txt, 在默认路径下 ( 用户和文件建立关系, 需要使用触发器给引用计数+1) 上传时间比如是 2023-04-18 20:26:12
#步骤1: 创建这个文件信息 引用计数是0  ,比如abc.txt 大小是 301字节 , md5 是13523afe…
...
#编写触发器
#需求:
#插入用户文件关系的时候, 对对应的id的文件信息, 文件引用+1;
#插入语句:
insert into t_user_file ( u_id , f_id , f_dir , f_name , f_uploadtime ) values( %d , %d , '%s' , '%s' , '%s' );  //参数 分别是 u_id  f_id  f_dir , f_name , f_uploadtime
...
#编写触发器
#需求:
#1.当删除t_user_file 的表中数据时, 比如 u_id的 f_dir路径的f_id 文件, 删除该条记录, #那么 t_file 对应的文件的引用计数-1
#2.如果引用计数达到0 , 那么删除t_file表中该条数据. 
#那么需要两个触发器.
#写好触发器之后, 服务端删除关联关系, 在t_file表中会自动改变引用计数.
#答:
#删除语句
